...
const DOT_COLORS: Record<string, string> = {
  ...
  assemblage: '#7C3AED', perte: '#D97706', bottling: '#0891B2', default: '#6B7280'
};
        ...
        {stats?.movement_types?.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {stats.movement_types.slice(0, 4).map((mt: any) => {
              ...
              return (
                <div key={mt.movement_type} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{mt.count}</p>
            ...
            <table className="w-full min-w-[600px]">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">De / Vers</th>
                  ...
                  <tr>
                    <td colSpan={7} className="text-center py-16">
                      <div className="flex flex-col items-center gap-3 text-gray-400">
                        ...
                        <p className="text-sm font-medium text-gray-500">Aucun mouvement enregistré</p>
                        <p className="text-xs text-gray-400">Commencez par enregistrer un mouvement de liquide</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={m.id} className="hover:bg-gray-50 transition-colors duration-100">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600">
                        {m.from_container_code && (
                          <span className="font-medium text-gray-700">{m.from_container_code}</span>
                        )}
                        {m.from_container_code && m.to_container_code && (
                          <span className="mx-1.5 text-gray-400">→</span>
                        )}
                        {m.to_container_code && (
                          <span className="font-medium text-gray-700">{m.to_container_code}</span>
                        )}
                        {!m.from_container_code && !m.to_container_code && (
                      ...
                      <td className="px-4 py-3">
                        {m.lot_number ? (
                          <div>
                            <p className="text-sm font-medium text-gray-700">{m.lot_number}</p>
                            {m.lot_name && <p className="text-xs text-gray-400">{m.lot_name}</p>}
                          </div>
                        ) : (
                      ...
                      <td className="px-4 py-3 text-sm text-gray-500 max-w-xs truncate">
                        {m.reason || <span className="text-gray-400">—</span>}
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-500">
                        {m.operator_name || <span className="text-gray-400">—</span>}
                      </td>
                    </tr>
                  ))
                )}
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
            {isLoading ? (
              ...
              <div className="text-center py-12 text-gray-400">
                ...
                <p className="text-sm text-gray-500">Aucun mouvement à afficher</p>
              </div>
            ) : (
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 flex-wrap">
                                <MovementBadge type={m.movement_type} />
                                {m.lot_number && (
                                  <span className="text-sm font-medium text-gray-700">{m.lot_number}</span>
                                )}
                            ...
                            <div className="text-right shrink-0">
                              ...
                              {m.operator_name && (
                                <p className="text-xs text-gray-400 mt-0.5">{m.operator_name}</p>
                              )}
                            </div>
                          </div>
                        </div>
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl border border-gray-200 shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Enregistrer un mouvement</h2>
          <button
            onClick={onClose}
